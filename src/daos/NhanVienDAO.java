...
public class NhanVienDAO extends DAOs<NhanVien, String> {
    ...
    public int insert(NhanVien e) {
        int i = -1;
        String sql = "INSERT INTO NhanVien (MaNV, MatKhau, HoVaTen, MaCV,NgaySinh,GioiTinh,DiaChi, SDT, Email, Hinh) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try {
            i = XJdbc.IUD(
                    sql,
                    e.getMaNV(), e.getMatKhau(),e.getHoVaTen(),e.getCv(),e.getNgaySinh(), e.isGioiTinh(), e.getDiaChi(),
